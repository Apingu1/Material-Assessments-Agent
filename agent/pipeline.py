...
def _all_hazard_sources(bundle: ResearchBundle) -> list[EvidenceSource]:
    items = [
        bundle.hazard.mutagenicity_genotoxicity,
        bundle.hazard.carcinogenicity,
        bundle.hazard.reproductive_developmental_toxicity,
        bundle.hazard.sensitisation_potential,
    ]
    return [source for item in items for source in item.sources]
...
def _appendix_range(captures: list[EvidenceCapture]) -> str:
    labels = [capture.appendix_label for capture in captures]
    if not labels:
        return ""
    if len(labels) == 1:
        return labels[0]
...
def _short_reference(captures: list[EvidenceCapture]) -> str:
    if not captures:
        return "No direct source - operator review"
    names: list[str] = []
    ...
    return f"{' / '.join(names)}{extra} - See App. {_appendix_range(captures)}"
...
class AssessmentPipeline:
    ...
    def run(self, item: MaterialInput) -> tuple[ResearchBundle, Path]:
        ...
        groups: list[tuple[str, int, list[EvidenceSource]]] = [
            ("Hazard", 1, _all_hazard_sources(bundle)),
            ("Potency", 2, bundle.potency.sources),
            ("Water Solubility", 3, bundle.cleanability.water.sources),
            ("70% IPA Solubility", 4, bundle.cleanability.ipa70.sources),
            ("2% Decon Solubility", 5, bundle.cleanability.decon2.sources),
            ("Physical Cleanability", 6, bundle.cleanability.physical.sources),
        ]

        captures: list[EvidenceCapture] = []
        refs: dict[str, str] = {}
        for group, number, sources in groups:
            group_captures = self.evidence.capture_group(group, number, sources, evidence_dir)
        ...
        append_evidence_dossier(filled_form, dossier, captures)
