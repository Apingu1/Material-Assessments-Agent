...
The controlled material name is not automatically used as the search identity for every endpoint. The identity stage resolves:

- controlled material name — remains exactly as entered on F01;
- chemical identity — species used for physicochemical/solubility research, retaining meaningful salt/hydrate/form while removing strength/presentation;
- active moiety — therapeutic identity used for clinical dose/use research across relevant strengths/formulations;
- process material — what physically contacts equipment and is used for physical cleanability.

For example, `Levothyroxine Sodium Powder` remains the controlled name, while clinical dose research uses levothyroxine and physicochemical research uses levothyroxine sodium. `Haloperidol 10mg Tablets` remains the controlled name, while clinical/hazard research is not restricted to 10 mg and physical cleanability can use the crushed-tablet/suspension process state.
...
Unless the supplied product/context explicitly identifies a paediatric or neonatal application, the agent defaults to the lowest routine adult therapeutic daily dose for the relevant route. Incoming strength and presentation do not limit clinical dose research.

A source can be scientifically valid even if it cannot be automatically screenshotted. Research validity and appendix-capture success are therefore handled separately. BNF may establish or support the dose conclusion even when its webpage blocks automated capture; another authoritative UK source can then be used as the visible appendix evidence.

### Canonical potency evidence waterfall

If no verified potency appendix evidence survives the initial capture attempt, the agent follows this fixed order:

1. NICE guidance / NICE-hosted clinical evidence;
2. UK eMC/SmPC;
3. MHRA/EMA;
4. FDA/DailyMed;
5. other Tier 1 evidence.

Each step searches the active ingredient across relevant route-appropriate strengths/formulations and stops as soon as suitable verified evidence is captured. The scientific conclusion is not changed merely to obtain an easier screenshot.
...
### Canonical cleanability evidence waterfalls

Where a cleanability score/conclusion exists but preferred evidence cannot be verified/captured, the application searches fixed endpoint-specific source families rather than making a single generic retry. Water/IPA search British/European pharmacopoeial evidence first, followed by PubChem, PubMed/PMC, DrugBank and established manufacturer technical information. Decon first checks direct material-in-Decon evidence before falling back to those material-property sources. Physical cleanability prioritises process/product information before regulatory product literature and manufacturer information.
...
- Water: 1.
- 70% IPA: 1.
- 2% Decon: up to 2 only where needed for a material-solvent inference.
- Physical cleanability: 1.

### Source-document deduplication

The physical appendix is deduplicated by canonical source-document URL, not by URL-plus-quotation. If the same document supports several endpoints, it is appended once and cross-referenced from all relevant form fields. Different relevant findings/interpretations from that document are consolidated into the single appendix entry rather than creating repeated screenshots.

Failed, blocked, 403, 404 or irrelevant screenshots are not included in the dossier; technical diagnostics remain in the evidence folder. PubMed capture uses an official NCBI text fallback when the standard webpage blocks automated capture.
